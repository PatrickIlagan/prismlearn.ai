...
export function S8Gamification() {
  ...
  const captionOpacity = interpolate(frame, [72, 84], [0, 1], { extrapolateLeft: "clamp", extrapolateRight: "clamp" });

  const xpT = interpolate(frame, [4, 34], [0.2, 1], { extrapolateLeft: "clamp", extrapolateRight: "clamp" });
  const burst = spring({ frame: frame - 34, fps, config: { damping: 9 }, durationInFrames: 16 });
  const burstOpacity = interpolate(frame, [34, 40, 55], [0, 1, 0], { extrapolateLeft: "clamp", extrapolateRight: "clamp" });

  const streakScale = spring({ frame: frame - 48, fps, config: { damping: 12 }, durationInFrames: 16 });
  const bossScale = spring({ frame: frame - 60, fps, config: { damping: 12 }, durationInFrames: 16 });
